...
<template>
  ...
  <InitialTable
    :friend="user"
    v-if="value == 'author' && authorInitial.length > 0"
    selectMode="single"
    :puzzleList="authorInitial"
  >
  </InitialTable>
  <NoDataToDisplay
    v-if="value == 'author' && authorInitial.length <= 0"
    customMessage="Korisnik nije autor niti jedne inicijalne osmosmjerke"
  >
  </NoDataToDisplay>
  <InitialTable
    :friend="user"
    v-if="value == 'updater' && updaterInitial.length > 0"
    selectMode="single"
    :puzzleList="updaterInitial"
  >
  </InitialTable>
  <NoDataToDisplay
    v-if="value == 'updater' && updaterInitial.length <= 0"
    customMessage="Korisnik nije zadnji ažurirali niti jednu inicijalnu osmosmjerku"
  >
  </NoDataToDisplay>
  <InitialTable
    :friend="user"
    v-if="value == 'collaborator' && collaboratorInitial.length > 0"
    selectMode="single"
    :puzzleList="collaboratorInitial"
  >
  </InitialTable>
  <NoDataToDisplay
    v-if="value == 'collaborator' && collaboratorInitial.length <= 0"
    customMessage="Korisnik nije suradnik niti na jednoj inicijalnoj osmosmjerci"
  >
  </NoDataToDisplay>
  <InitialTable
    :friend="user"
