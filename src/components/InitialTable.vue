...
<script> 
    ...
    import { initialRecordsRef } from "../main.js"
    ...
    export default { 
        ...
        data() {
            return { 
                user: null,
                initialRecordsRef: initialRecordsRef,
                puzzles: [], 
        ...
        methods: {
            ...
            fetch_puzzles() {
                ...
                initialRef.get().then(function(snapshot) {
...
<template>   
    ...
    <span v-if="!start_time && !end_time && selectMode == 'single'"> 
        <span v-for="item in selectedItemsEmitted" :key="item.id"> 
            <br>
            <h2 class="display-2">Svi rekordi za odabranu brojevnu križaljku</h2>
            <RecordsTable :dbRef="initialRecordsRef" :puzzleId="selectedItemsEmitted[0].id"></RecordsTable>
            <br>
            <span v-if="user">
                <h2 class="display-2">Moji rekordi za odabranu brojevnu križaljku</h2>
                <RecordsTable :dbRef="initialRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RecordsTable>
            </span>
        </span>
    </span>
    <span v-if="start_time && end_time && selectMode == 'single'"> 
        <span v-for="item in selectedItemsEmitted" :key="item.id"> 
            <br>
            <h2 class="display-2">Svi rekordi za odabranu inicijalnu osmosmjerku</h2>
            <RecordsTable :dbRef="initialRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :start_time="start_time" :end_time="end_time"></RecordsTable>
            <br>
            <span v-if="user">
                <h2 class="display-2">Moji rekordi za odabranu inicijalnu osmosmjerku</h2>
                <RecordsTable :dbRef="initialRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid" :start_time="start_time" :end_time="end_time"></RecordsTable>
            </span>
        </span>
    </span>  
</template>
