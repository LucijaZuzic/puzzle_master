...
<script> 
    ...
    import { initialsRecordsRef, initialsRatingsRef } from "../main.js"
    ...
    export default { 
        ...
        methods: {
                ...
                initialsRef.get().then(function(snapshot) {
                    snapshot.forEach(function(childSnapshot) { 
                        ...
                        if (inList == true) {
                            let sum_ratings = 0
                            let num_ratings = 0
                            initialsRatingsRef.get().then(function(snapshotRating) {
            ...
            deletePuzzle(id) {
                initialsRef.doc(id).delete().then(() => {
                    this.fetch_puzzles() 
                    this.$forceUpdate()
                })
            }
...
<template>   
    ...
    <va-data-table 
        ...
        @selectionChange="selectedItemsEmitted = $event.currentSelectedItems;$emit('selectedInitials', selectedItemsEmitted)"
    ...
    <div class="myrow" v-if="!start_time && !end_time && selectMode == 'single'"> 
        <va-tabs v-if="selectedItemsEmitted.length > 0" v-model="value" vertical>
            <template #tabs> 
            <va-tab
                label="Svi rekordi za odabranu inicijalnu osmosmjerku"
                name="all"
            />
            <va-tab
                label="Rekordi korisnika za odabranu inicijalnu osmosmjerku"
                name="mine"
            />
            <va-tab
                label="Ocjena za inicijalnu osmosmjerku"
                name="rate"
            />
            </template>
        </va-tabs>
        <span v-for="item in selectedItemsEmitted" :key="item.id">  
            <RatingsTable v-if="value=='rate'" :dbRef="initialsRatingsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RatingsTable>
            <RecordsTable v-if="value=='all'" :dbRef="initialsRecordsRef" :puzzleId="selectedItemsEmitted[0].id"></RecordsTable> 
            <RecordsTable v-if="user && value=='mine'" :dbRef="initialsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RecordsTable>
        </span>
    </div> 
    <div class="myrow" v-if="start_time && end_time && selectMode == 'single'"> 
        <va-tabs v-if="selectedItemsEmitted.length > 0" v-model="value" vertical>
            <template #tabs> 
            <va-tab
                label="Svi rekordi za odabranu inicijalnu osmosmjerku unutar vremena turnira"
                name="all"
            />
            <va-tab
                label="Rekordi korisnika za odabranu inicijalnu osmosmjerku unutar vremena turnira"
                name="mine"
            />
            <va-tab
                label="Ocjena za inicijalnu osmosmjerku"
        ...
        <span v-for="item in selectedItemsEmitted" :key="item.id">  
            <RatingsTable v-if="value=='rate'" :dbRef="initialsRatingsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RatingsTable>
            <RecordsTable v-if="value=='all'" :dbRef="initialsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :start_time="start_time" :end_time="end_time"></RecordsTable> 
            <RecordsTable v-if="user && value=='mine'" :dbRef="initialsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid" :start_time="start_time" :end_time="end_time"></RecordsTable>
        </span>
    </div>   
</template>
