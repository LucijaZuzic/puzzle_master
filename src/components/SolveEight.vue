...
export default {
  ...
  data() {
    return {
        ...
        last_updated: '',
        is_special: [], 
        alphabet: ["A", "B", "C", "Č", "Ć", "D", "Đ", "Ǆ", "E", "F", "G", "H", "I", "J", "K", "L", "Ǉ", "M", "N", "Ǌ", "O", "P", "Q", "R", "S", "Š", "T", "U", "V", "W", "X", "Y", "Z", "Ž"],
        description: '',
        current_x: null,
        current_y: null
    }
  ...
  methods: { 
      ...
      show_solution() {
        ...
        for (let i = 0; i < this.words_by_dir.length; i++) { 
            this.placed_words.push([])
            for (let j = 0; j < this.words_by_dir[i].length; j++) {
                this.placed_words.push([this.words_by_dir[i][j][0], this.words_by_dir[i][j][1]])
            }
        }
        this.cheat = true;
        this.check_victory();
      ...
      fetch_puzzle() {
        ...
        eightsRef.get(params_id).then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                let id = childSnapshot.id;
                if (id == params_id) {
            ...
            if (found) {
                ...
                this.new_async(this.$vaToast.init("Ne postoji zagonetka s tim brojem."), 1000).then(() => {this.$router.push("/search-eight")})  
            }
        })
      },
            ...
            for (let j = 0; j < this.words_by_dir[i].length; j++) {
                if (this.words_by_dir[i][j][0] == x && this.words_by_dir[i][j][1] == y) {
                    if (show_warning == true) {
                        this.$vaToast.init("Nova riječ ne može započeti na istom polju kao već dodana riječ.") 
                    }
                    return true
                }
            }
      ...
      check_dir(x, y, word_index, dirx, diry, show_warning) {
          ...
          for (let letter_number = 0; letter_number < new_word.length; letter_number++) {   
              ...
              if (x_new < 0 || x_new >= this.columns) {
                  if (show_warning) {
                    this.$vaToast.init("Nova riječ ne može započeti na odabranom polju jer u odabranom smjeru nema dovoljno polja.")
                  }
                  return false
              }
...
<template>
  ...
  <body class="mybody" v-else>
    ...
    <div class="myrow">    
        <va-button style="overflow-wrap:anywhere" @click="reset()"><va-icon name="delete"/>&nbsp;Izbriši</va-button>
    </div> 
    <div class="myrow"> 
            ...
            <div> 
                <table class="words_table">
                    <tr v-for="i in (rows)" v-bind:key="i">
                        <td v-for="j in (columns)" v-bind:key="j" @click="place_word(j-1,i-1,word_index,mode_x,mode_y,true)" @mouseover="current_x = i; current_y = j"  
                        :class="{wrong: values[i - 1][j - 1] != is_special[i - 1][j - 1] && show_error, 
    ...
    <div class="myrow" v-if="count_special()">
        ...
        <span v-for="i in (rows)" v-bind:key="i">
            <span v-for="j in (columns)" v-bind:key="j">
                <va-chip v-if="values[i - 1][j - 1] && values[i - 1][j - 1] != is_special[i - 1][j - 1] && show_error" color="danger" outline square style="margin-left:5px; margin-top:5px"> 
                    {{solution[i - 1][j - 1]}}
                </va-chip>
                <va-chip v-if="values[i - 1][j - 1] && !(values[i - 1][j - 1] != is_special[i - 1][j - 1] && show_error)" outline square style="margin-left:5px; margin-top:5px"> 
                    {{solution[i - 1][j - 1]}}
                </va-chip>
            </span>
        </span>
    </div> 
    <div class="myrow" v-if="words_by_dir[0].length + words_by_dir[1].length + words_by_dir[2].length + words_by_dir[3].length
    + words_by_dir[4].length + words_by_dir[5].length + words_by_dir[6].length + words_by_dir[7].length > 0">
        <va-infinite-scroll disabled :load="()=> {}">
            <div style="max-height: 320px">
                <span v-for="(words_in_dir, i) in words_by_dir" v-bind:key="i">
                    <va-chip outline v-if="words_in_dir.length > 0" style="padding: 20px; margin-left: 20px; margin-top: 20px">
                        <span>
                            ...
                            <div style="max-height: 200px">
                                <va-infinite-scroll disabled :load="()=> {}">
                                        <div v-for="(word, j) in words_in_dir" v-bind:key="j">
                                            <span @click="select_word(i, j)"
                                            :class="{selected: word_index == j && get_dir(mode_x, mode_y) == i, placed: placed_words[i][j] != 0}">
                                                {{word[2]}}
                                            </span>
                                            &nbsp;
