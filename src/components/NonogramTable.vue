...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: "colors", sortable: true, classes: "data_table_overflow" },
  ...
  methods: {
    ...
    add_favorite(id) {
      this.favoriteNonograms.push(id);
      usersRef
        .doc(this.user.uid)
        .set(
          {
            favoriteNonograms: this.favoriteNonograms,
          },
          { merge: true }
        )
        .then(() => {
    ...
    remove_favorite(id) {
      let index = this.favoriteNonograms.indexOf(id);
      if (index > -1) {
        this.favoriteNonograms.splice(index, 1);
      }
      usersRef
        .doc(this.user.uid)
        .set(
    ...
    <span v-if="puzzles.length > 0">
      ...
      >
        ...
        <template #cell(id)="{ source: id }">
          <router-link
            v-bind:to="{ name: 'solve_nonogram', params: { id: id } }"
          >
