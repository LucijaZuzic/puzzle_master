...
<template>
  ...
  <NumberLetterTable
    :friend="user"
    v-if="value == 'author' && authorNumberLetter.length > 0"
    selectMode="single"
    :puzzleList="authorNumberLetter"
  >
  </NumberLetterTable>
  <NoDataToDisplay
    v-if="value == 'author' && authorNumberLetter.length <= 0"
    customMessage='Korisnik nije autor niti jedne zagonetke tipa "Isti broj - Isto slovo"'
  >
  </NoDataToDisplay>
  <NumberLetterTable
    :friend="user"
    v-if="value == 'updater' && updaterNumberLetter.length > 0"
    selectMode="single"
    :puzzleList="updaterNumberLetter"
  >
  </NumberLetterTable>
  <NoDataToDisplay
    v-if="value == 'updater' && updaterNumberLetter.length <= 0"
    customMessage='Korisnik nije zadnji ažurirali niti jednu zagonetku tipa "Isti broj - Isto slovo"'
  >
  </NoDataToDisplay>
  <NumberLetterTable
    :friend="user"
    v-if="value == 'collaborator' && collaboratorNumberLetter.length > 0"
    selectMode="single"
    :puzzleList="collaboratorNumberLetter"
  >
  </NumberLetterTable>
  <NoDataToDisplay
    v-if="value == 'collaborator' && collaboratorNumberLetter.length <= 0"
    customMessage='Korisnik nije suradnik niti na jednoj zagonetki tipa "Isti broj - Isto slovo"'
  >
  </NoDataToDisplay>
  <NumberLetterTable
    :friend="user"
