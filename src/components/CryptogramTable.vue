...
<script>
    ...
    import { cryptogramsRecordsRef, cryptogramsRatingsRef } from "../main.js"
...
<template>  
    ...
    <va-data-table 
        ...
        @selectionChange="selectedItemsEmitted = $event.currentSelectedItems;$emit('selectedCryptograms', selectedItemsEmitted)"
        ...
        <template #cell(id)="{ source: id }">
            <router-link v-bind:to="{ name: 'edit_cryptogram', params: { id: id }}">
                <va-icon name="mode_edit"/>
            </router-link>
            <router-link v-bind:to="{ name: 'solve_cryptogram', params: { id: id }}">
    ...
    <div class="myrow" v-if="!start_time && !end_time && selectMode == 'single'"> 
        ...
        <span v-for="item in selectedItemsEmitted" :key="item.id">  
            <RatingsTable v-if="value=='rate'" :dbRef="cryptogramsRatingsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RatingsTable>
            <RecordsTable v-if="value=='all'" :dbRef="cryptogramsRecordsRef" :puzzleId="selectedItemsEmitted[0].id"></RecordsTable> 
            <RecordsTable v-if="user && value=='mine'" :dbRef="cryptogramsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RecordsTable>
        </span>
    </div> 
    <div class="myrow" v-if="start_time && end_time && selectMode == 'single'"> 
        <va-tabs v-if="selectedItemsEmitted.length > 0" v-model="value" vertical>
        ...
        <span v-for="item in selectedItemsEmitted" :key="item.id">  
            <RatingsTable v-if="value=='rate'" :dbRef="cryptogramsRatingsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid"></RatingsTable>
            <RecordsTable v-if="value=='all'" :dbRef="cryptogramsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :start_time="start_time" :end_time="end_time"></RecordsTable> 
            <RecordsTable v-if="user && value=='mine'" :dbRef="cryptogramsRecordsRef" :puzzleId="selectedItemsEmitted[0].id" :userId="user.uid" :start_time="start_time" :end_time="end_time"></RecordsTable>
        </span>
    </div>  
</template>
